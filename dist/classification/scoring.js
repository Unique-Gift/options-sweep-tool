...
exports.scoreContract = scoreContract;
function scoreContract(contract, baseline, currentPrice, daysToExpiration, strikeOI, context) {
    ...
    const composite = Math.round(volumeAnomaly * 0.3 +
        orderCharacter * 0.25 +
        ivContext * 0.2 +
        strikeSelection * 0.15 +
        gexPosition * 0.1);
    const signal = composite >= 62
...
function scoreVolumeAnomaly(contract, baseline, context) {
    let score = 50;
    ...
    if (context) {
        if (context.relativeVolume >= 2)
            score = Math.min(100, score + 10);
        else if (context.relativeVolume < 0.5)
            score = Math.max(0, score - 10);
    }
    return score;
}
function scoreOrderCharacter(contract) {
...
function scoreIVContext(contract, baseline) {
    if (contract.impliedVolatility === 0)
        return 50;
    if (!baseline)
        return 50;
    if (baseline.ivPercentile <= 20)
        return 100;
    if (baseline.ivPercentile <= 40)
        return 75;
    if (baseline.ivPercentile <= 60)
        return 50;
    if (baseline.ivPercentile <= 80)
        return 25;
    return 10;
}
function scoreStrikeSelection(contract, currentPrice, daysToExpiration) {
    const distancePercent = (Math.abs(contract.strike - currentPrice) / currentPrice) * 100;
    const isShortDated = daysToExpiration <= 7;
    if (distancePercent >= 5 && isShortDated)
        return 100;
    if (distancePercent >= 3 && isShortDated)
        return 80;
