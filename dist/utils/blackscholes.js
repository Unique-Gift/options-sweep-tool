...
exports.calculateIV = calculateIV;
function calculateIV(optionPrice, stockPrice, strike, daysToExpiration, optionType, riskFreeRate = 0.05) {
    if (optionPrice <= 0 || daysToExpiration <= 0)
        return 0;
    ...
    for (let i = 0; i < 100; i++) {
        ...
        if (Math.abs(price - optionPrice) < 0.001)
            return mid;
